  ...
  <div>
    ...
    <ul v-else>
      <li v-for="article in articles" :key="article.id" class="mb-6 flex flex-col">
        <RouterLink :to="{ name: 'article-detail', params: { id: article.id } }" class="w-full aspect-[16/9] overflow-hidden rounded-lg mb-2 block">
          <img
        ...
        <div class="flex flex-col gap-1">
          <RouterLink :to="{ name: 'article-detail', params: { id: article.id } }" class="text-base font-bold text-gray-900 leading-tight mb-1 line-clamp-2 hover:text-blue-600 transition">
            {{ article.title }}
          </RouterLink>
          <div class="text-xs text-gray-500 mb-1">{{ formatDate(article.published_at) }}</div>
          <RouterLink :to="{ name: 'article-detail', params: { id: article.id } }" class="text-gray-600 text-sm line-clamp-2 mb-1 hover:text-blue-600 transition">
            {{ article.summary }}
          </RouterLink>
